...
use crate::loom_api::sync::atomic::{AtomicBool, Ordering};
...
impl<const S: usize> JobStorage<S> {
    ...
    unsafe fn store<T>(&self, val: T) {
        unsafe {
            self.buf
                .with_mut(|buf| ((*buf).as_mut_ptr() as *mut T).write(val));
            self.drop_fn.with_mut(|df| *df = Some(Self::drop_glue::<T>));
        }
    }
    ...
    unsafe fn take<T>(&self) -> T {
        unsafe {
            let val = self
                .buf
                .with(|buf| ((*buf).as_ptr() as *const T).read());
            self.drop_fn.with_mut(|df| *df = None);
            val
        }
    }
    ...
    unsafe fn drop_contents(&self) {
        unsafe {
            let f = self.drop_fn.with_mut(|df| (*df).take());
            if let Some(f) = f {
                f(self);
            }
        }
    ...
    unsafe fn drop_glue<T>(slot: &Self) {
        unsafe {
            slot.buf
                .with_mut(|buf| core::ptr::drop_in_place((*buf).as_mut_ptr() as *mut T));
        }
    }
}
...
unsafe fn run_job<T, R, F: FnOnce(&mut T) -> R, const S: usize>(
    slot: &JobStorage<S>,
    state: &mut T,
) {
    unsafe {
...
mod tests {
    ...
    async fn miri_heterogeneous_types_no_ub() {
        ...
        let caller = async {
            ...
            assert_eq!(len, 3);
            let s: String = svc
                .call(|s| String::from_utf8(s.clone()).unwrap_or_default())
                .await;
    ...
    fn threaded_different_return_types() {
        ...
        let pool = ThreadPool::new().unwrap();
        let svc =
            Arc::new(ContextService::<CriticalSectionRawMutex, Vec<String>, 256>::new());
...
mod loom_tests {
    ...
    use super::*;
    use crate::blocking_mutex::raw::LoomRawMutex;
    use core::pin::pin;

    /// Minimal block_on that works under loom.
    /// Parks the current loom thread when pending; wakers call unpark,
    /// giving loom proper scheduling edges to explore.
    fn block_on<F: core::future::Future>(f: F) -> F::Output {
        use core::task::{RawWaker, RawWakerVTable, Waker};

        let thread = loom::thread::current();
    ...
    /// Minimal select that returns when *either* future completes.
    /// Unlike futures_util::select, this doesn't need an external dep and
    /// works correctly with loom's waker model (both branches share the
    /// same waker → same thread unpark).
    async fn select_biased<A: Future, B: Future>(a: A, b: B) -> SelectResult<A::Output, B::Output> {
        let mut a = pin!(a);
        let mut b = pin!(b);
        core::future::poll_fn(|cx| {
            if let Poll::Ready(val) = a.as_mut().poll(cx) {
                return Poll::Ready(SelectResult::A(val));
            }
            if let Poll::Ready(val) = b.as_mut().poll(cx) {
                return Poll::Ready(SelectResult::B(val));
            }
            Poll::Pending
        })
        .await
    ...
    /// Drive the runner on a separate thread using proper wakers.
    /// The runner is wrapped in a select with a stop signal so it
    /// terminates when the stop signal fires.
    ///
    /// Returns a JoinHandle. Call `stop.signal(())` to shut down the runner,
    /// then join the handle.
    fn spawn_runner<T: Send + 'static>(
        svc: Arc<ContextService<LoomRawMutex, T, 64>>,
        mut state: T,
    ) -> (loom::thread::JoinHandle<T>, Arc<Signal<LoomRawMutex, ()>>) {
        let stop = Arc::new(Signal::<LoomRawMutex, ()>::new());
        let stop2 = stop.clone();
        let handle = loom::thread::spawn(move || {
            block_on(async {
                select_biased(svc.run(&mut state), stop2.wait()).await;
            });
            state
        });
        (handle, stop)
    ...
    #[test]
    fn loom_job_storage_store_take() {
        loom::model(|| {
            let storage = JobStorage::<64>::new();
            unsafe {
                storage.store(42u32);
                let val: u32 = storage.take();
                assert_eq!(val, 42);
            }
        });
    }
    ...
    #[test]
    fn loom_job_storage_drop_contents() {
        loom::model(|| {
            let dropped = Arc::new(loom::sync::atomic::AtomicBool::new(false));
            let dropped_clone = dropped.clone();

            struct Tracked(Arc<loom::sync::atomic::AtomicBool>);
            impl Drop for Tracked {
                fn drop(&mut self) {
            ...
            let storage = JobStorage::<64>::new();
            unsafe {
                storage.store(Tracked(dropped_clone));
                assert!(!dropped.load(Ordering::Relaxed));
                storage.drop_contents();
                assert!(dropped.load(Ordering::Relaxed));
            }
        });
    }
    ...
    #[test]
    fn loom_concurrent_running_flag() {
        loom::model(|| {
            let svc = Arc::new(ContextService::<LoomRawMutex, i32, 64>::new());

            let svc2 = svc.clone();
            let t1 = loom::thread::spawn(move || {
                let was_running = svc2.running.swap(true, Ordering::Acquire);
                if !was_running {
                    svc2.running.store(false, Ordering::Release);
                }
                was_running
            });
            ...
            let t2 = loom::thread::spawn(move || {
                let was_running = svc3.running.swap(true, Ordering::Acquire);
                if !was_running {
                    svc3.running.store(false, Ordering::Release);
                }
                was_running
            });
            ...
            assert!(!(r1 && r2), "both threads saw running=true, impossible");
        });
    }

    // ── Cross-thread call/run tests (proper wakers) ─────────────────

    /// Caller on one loom thread, runner on another. Both use real
    /// block_on with park/unpark wakers, so loom explores the actual
    /// wake-based scheduling edges in the signal protocol.
    #[test]
    fn loom_cross_thread_call() {
        loom::model(|| {
            let svc = Arc::new(ContextService::<LoomRawMutex, i32, 64>::new());

            let (runner, stop) = spawn_runner(svc.clone(), 0i32);

            let result = block_on(svc.call(|s| {
                *s += 42;
                *s
            }));
            assert_eq!(result, 42);
            ...
            stop.signal(());
            let final_state = runner.join().unwrap();
            assert_eq!(final_state, 42);
        });
    }

    // NOTE: Two-callers-one-runner removed — 3 loom threads with
    // the full Signal/Semaphore protocol creates too many interleavings.
    // try_call_immediate_races_call covers permit contention instead.
    #[test]
    #[ignore] // state space too large for CI; run manually with --ignored
    fn loom_two_callers_one_runner() {
        loom::model(|| {
            let svc = Arc::new(ContextService::<LoomRawMutex, i32, 64>::new());
            ...
            assert!(
                (ra == 1 && rb == 2) || (ra == 2 && rb == 1),
                "unexpected results: ra={ra}, rb={rb}"
            );
    ...
    /// Cancel a call future after it has been submitted (phase = Submitted),
    /// then make a new call. The Drop impl signals ack, and the runner
    /// must clean up before processing the next job. All with real wakers.
    #[test]
    fn loom_cancel_after_submit() {
        loom::model(|| {
            let svc = Arc::new(ContextService::<LoomRawMutex, i32, 64>::new());
            ...
            block_on(async {
                // Submit a call and immediately drop it.
                {
                    let fut = svc.call(|s: &mut i32| {
                        *s += 100;
                        *s
                    ...
                    let mut fut = pin!(fut);
                    // Poll once — may or may not reach Submitted depending
                    // on whether the runner has initialized the permit yet.
                    let waker = futures_util::task::noop_waker();
                    let mut cx = core::task::Context::from_waker(&waker);
                    let _ = fut.as_mut().poll(&mut cx);
                    // Drop: if Submitted, signals ack.
                }

                loom::thread::yield_now();

                // Second call must succeed — the service must not deadlock
                // or lose the permit after a cancellation.
                let r = svc
                    .call(|s| {
                        *s += 1;
                        *s
                    ...
                    .await;
                assert!(r > 0, "unexpected result: {r}");
            });
    ...
    // ── try_call_immediate tests ────────────────────────────────────

    /// try_call_immediate on one thread while the runner processes on another.
    #[test]
    fn loom_try_call_immediate() {
        loom::model(|| {
            let svc = Arc::new(ContextService::<LoomRawMutex, i32, 64>::new());

            let (runner, stop) = spawn_runner(svc.clone(), 0i32);

            // May or may not succeed depending on whether the runner
            // has initialized the permit yet.
            let _ = svc.try_call_immediate(|s| {
                *s += 7;
            });
    ...
    /// try_call_immediate racing against a normal async call for the
    /// same single permit. Exactly one should win; the other must
    /// either fail (try_call_immediate) or block until the first completes.
    #[test]
    fn loom_try_call_immediate_races_call() {
        loom::model(|| {
            let svc = Arc::new(ContextService::<LoomRawMutex, i32, 64>::new());
            ...
            let svc2 = svc.clone();
            let immediate = loom::thread::spawn(move || {
                svc2.try_call_immediate(|s| {
                    *s += 10;
                })
            });
            ...
            let caller = loom::thread::spawn(move || {
                block_on(svc3.call(|s| {
                    *s += 1;
                    *s
                }))
            });

            let imm_ok = immediate.join().unwrap();
            let call_result = caller.join().unwrap();

            // Both operations must eventually complete without deadlock.
            // The key invariant: no deadlock, no UB, call completes.
            // The exact result depends on scheduling order.
            assert!(
                call_result > 0,
                "call must complete with a positive result, got {call_result}"
            );
            let _ = imm_ok;
    ...
    // ── Runner restart / recovery tests ─────────────────────────────

    /// Drop the runner mid-job (after done, waiting for ack), restart
    /// with a new run(), verify recovery cleans up and service works.
    #[test]
    fn loom_runner_restart_recovery() {
        loom::model(|| {
            let svc = Arc::new(ContextService::<LoomRawMutex, i32, 64>::new());

            // First runner — will be cancelled.
            let (runner1, stop1) = spawn_runner(svc.clone(), 0i32);

            // Submit a call that the first runner will process.
            // Then cancel the first runner by signaling stop.
            let r = block_on(svc.call(|s| {
                *s += 100;
                *s
            }));
            assert_eq!(r, 100);

            // Stop the first runner. It may be cancelled at ack.wait()
            // or at job.wait() — either way, needs_recovery may be set.
            stop1.signal(());
            runner1.join().unwrap();

            // Second runner — must recover any in-flight state and work.
            let (runner2, stop2) = spawn_runner(svc.clone(), 0i32);

            let r2 = block_on(svc.call(|s| {
                *s += 1;
                *s
            }));
            // The second runner has a fresh state, so result is 1.
            assert_eq!(r2, 1);
    ...
    // ── Initialized flag test ───────────────────────────────────────

    /// Two sequential run() calls must not release the permit twice.
    /// Verify by checking the actual permit count.
    #[test]
    fn loom_initialized_flag_idempotent() {
        loom::model(|| {
            let svc = Arc::new(ContextService::<LoomRawMutex, i32, 64>::new());
            ...
            assert!(svc.initialized.load(Ordering::Relaxed));
            assert_eq!(svc.permit.permits(), 1);

            // Second run — must NOT release another permit.
            {
                let (runner, stop) = spawn_runner(svc.clone(), 0i32);
                loom::thread::yield_now();
                stop.signal(());
            ...
            assert!(svc.initialized.load(Ordering::Relaxed));
            // Still exactly 1 permit — not 2.
            assert_eq!(svc.permit.permits(), 1);
        });
    }

    // ── Heap value through slot ─────────────────────────────────────

    /// Pass a heap-allocated Box through the slot. Loom's UnsafeCell
    /// tracking verifies no data races on the slot accesses.
    #[test]
    fn loom_heap_value_through_slot() {
        extern crate alloc;
        use alloc::boxed::Box;

        loom::model(|| {
            let svc = Arc::new(ContextService::<LoomRawMutex, (), 64>::new());

            let (runner, stop) = spawn_runner(svc.clone(), ());

            let result: Box<u32> = block_on(svc.call(|_| Box::new(12345u32)));
